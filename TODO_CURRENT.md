...
- Implement the remaining Phase 3 slices: provider/cache/preflight/update
  runtime integration (cheap durable V2 availability instead of load-time
  rehash, lease sequence retirement, cache eviction provider updates,
  cache-disabled lease-only materialization), productive V2 transport
  activation, fault injection, stress validation, documentation closure, and
